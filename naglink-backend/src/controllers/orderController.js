...
const addOrderExpenses = async (req, res) => {
  try {
    ...
    await createNotification({
      roleTarget: "ceo",
      orderId: order.id,
      title: "New Expense Requisition",
      message: `Expense requisition for order ${
        order.orderNumber || `#${order.id}`
      } is waiting for CEO approval. Amount: $${totalAmount.toFixed(2)}.`,
      type: "expense_requisition",
    });

    res.status(201).json({
      message: "Expense requisition submitted. Waiting for CEO approval.",
      expense,
    });
  } catch (error) {
